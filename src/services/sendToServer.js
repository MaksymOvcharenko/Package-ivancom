...
const sendShipmentData = async (state) => {
  ...
  const gabarytes = {
    ...
    B: { length: 64, width: 38, height: 19, weightActual: 10 },
  ...
  const data = {
    ...
    payment: {
      ...
      promocode: checkPromoCode(parcel.promocode),
    },
  };
  ...
  try {
    const response = await axios.post(
      "https://ivancom-server.onrender.com/generate-package",
      data
    ); // Поменять на рендер апі
    console.log("Shipment successfully created:", response.data);
    return response;
  } catch (error) {
    console.error(
      "Error sending shipment data:",
